...
using AdvertismentApp.Common.Enums;
using AutoMapper;
...
namespace AdvertisementApp.UI.Controllers
{
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> SignIn(AppUserLoginDto dto)
		{
			...
			if (result.ResponseType == AdvertismentApp.Common.ResponseType.Success)
			{
				var roleResult = await _appUserService.GetRolesByUserIdAsync(result.Data.Id);
				// ilgili kullanıcının rollerini çekmemiz.
				var claims = new List<Claim>();

				if (roleResult.ResponseType ==AdvertismentApp.Common.ResponseType.Success)
				{
